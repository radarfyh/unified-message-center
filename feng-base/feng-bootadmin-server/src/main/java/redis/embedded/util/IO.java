...
public enum IO {;
    ...
    public static File writeResourceToExecutableFile(final File tempDirectory, final String resourcePath) throws IOException {
        ...
        log.info("writeResourceToExecutableFile tempDirectory: {}, resourcePath: {}", tempDirectory, resourcePath);
        ...
        if (resourcePath.contains("redis-server-3.2.100-windows.exe")) {
	        String rdbPath = "/redis/dump.rdb";
	        final File rdbFile = new File(tempDirectory, rdbPath);
	        try (final InputStream in = IO.class.getResourceAsStream(rdbPath)) {
	            if (in == null) throw new FileNotFoundException("Could not find Redis RDB at " + rdbPath);
	            Files.copy(in, rdbFile.toPath(), REPLACE_EXISTING);
	        }
        }
